...
# Fungsi rekursif untuk menghitung jarak kumulatif
def hitung_jarak_kumulatif_rekursif(jarak_setiap_tujuan, index=0, kumulatif=0, hasil=None):
    if hasil is None:
        hasil = []
    if index >= len(jarak_setiap_tujuan):
        return hasil
    kumulatif += jarak_setiap_tujuan[index]
    hasil.append(kumulatif)
    return hitung_jarak_kumulatif_rekursif(jarak_setiap_tujuan, index + 1, kumulatif, hasil)

# Fungsi iteratif untuk menghitung jarak kumulatif
def hitung_jarak_kumulatif_iteratif(jarak_setiap_tujuan):
    hasil = []
    kumulatif = 0
    for jarak in jarak_setiap_tujuan:
        kumulatif += jarak
        hasil.append(kumulatif)
    return hasil

# Fungsi untuk membuat grafik pengiriman
def buat_grafik_pengiriman(rute, jarak_iteratif, jarak_rekursif):
    plt.figure(figsize=(10, 6))
    plt.plot(rute, jarak_iteratif, marker='o', linestyle='-', color='b', label='Iteratif')
    plt.plot(rute, jarak_rekursif, marker='x', linestyle='--', color='r', label='Rekursif')
    plt.title("Grafik Pengiriman Berdasarkan Jarak (Iteratif vs Rekursif)")
...
if sum(berat_barang) > drone["kapasitas_berat"]:
    ...
    if total_jarak > drone["jarak_tempuh_maks"]:
        ...
        # Hitung jarak kumulatif dengan iteratif dan rekursif
        jarak_kumulatif_iteratif = hitung_jarak_kumulatif_iteratif(jarak_setiap_tujuan)
        jarak_kumulatif_rekursif = hitung_jarak_kumulatif_rekursif(jarak_setiap_tujuan)

        # Buat grafik pengiriman
        buat_grafik_pengiriman(rute_pengiriman, jarak_kumulatif_iteratif, jarak_kumulatif_rekursif)