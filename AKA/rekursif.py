...
def hitung_jarak_kumulatif(jarak_setiap_tujuan, index=0, kumulatif=0, hasil=None):
    if hasil is None:
        hasil = []
    ...
    return hitung_jarak_kumulatif(jarak_setiap_tujuan, index + 1, kumulatif, hasil)

# Fungsi untuk membuat grafik pengiriman
def buat_grafik_pengiriman(rute, jarak_kumulatif):
    plt.figure(figsize=(10, 6))
    plt.plot(rute, jarak_kumulatif, marker='o', linestyle='-', color='b')
    plt.title("Grafik Pengiriman Berdasarkan Jarak")
...
if sum(berat_barang) > drone["kapasitas_berat"]:
    ...
    if total_jarak > drone["jarak_tempuh_maks"]:
        ...
        # Hitung jarak kumulatif untuk grafik secara rekursif
        jarak_kumulatif = hitung_jarak_kumulatif(jarak_setiap_tujuan)

        # Buat grafik pengiriman
        buat_grafik_pengiriman(rute_pengiriman, jarak_kumulatif)